...
let Lobby = Backbone.View.extend({
  ...
  events: {
    ...
    "click .build-deck": "openDeckBuilder",
  },
  debugNote: function () {
...
let DeckBuilder = Backbone.View.extend({
    template: require("../templates/deck-builder.handlebars"),
  
    initialize: function(options) {
      this.availableCards = options.availableCards;
      this.customDeck = options.customDeck;
      // Add any other options you need, e.g., factions or event handlers for buttons
    },
  
    events: {
      // Add your event listeners here, for example:
      // "click .btn-save-deck": "saveDeck",
      // "click .btn-cancel-deck": "cancelDeck"
    },
  
    render: function() {
        console.log("DeckBuilder render called");
